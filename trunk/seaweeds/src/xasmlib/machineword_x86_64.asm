...
; PROC source:PTR, bitofs:BYTE, bits:BYTE
  ...
  ; load the data
  xor r8, r8
  mov rax, [rdi]	; rdi contains source
  mov r8,  [rdi+8]	; source is padded with >= 1 QWORD
  mov rcx, rsi  ; bitofs
  ; shift right into rax
  shrd rax, r8, cl
  shl rdx, 8
  lea rdx, [bitmasks_64 wrt rip]
  and rax, [rdx]
  RET
...
;PROC source:QWORD, target:ptr, bitofs:BYTE, bits:BYTE
  ...
  and rcx, 03fh
  and rdx, 03fh
  xchg cx, dx	; we need to shift by bitofs

  ; first part of bitmask: set the first bitofs bits
  lea r8, [bitmasks_64 wrt rip]
  mov r10, [8*rcx + r8]
  ...
  mov rax, [rsi]		; get vector data
  shl rdi, cl			; shift input into place
  and rax, r11			; make sure we only get the relevant bits
  not r11			; reset the bits in the vector data
  and rdi, r11			; ...
  or rax, rdi			; and put in our data
  mov [rsi], rax		; put qword back
  RET

.big_write:			; if a word crosses a qword boundary, this is what happens
  mov rax, [rsi]		; we first write the first half
  xor r9, r9			; for shifting, remember the bit that crosses the boundary
  shld r9, rdi, cl		; and hence goes into the next qword
  shl rdi, cl			; shift the first half into place
  and rax, r10			; remove the bits we will replace
  or rax, rdi			; put in the first half
  mov [rsi], rax		; write back

  lea r10, [bitmasks_64 wrt rip]
  mov r8, [r10 + 8*rdx] ; get second half of mask
  and r9, r8			; r9 contains the upper half of the word
  not r8			; invert write mask
  mov rax, [rsi+8]		; read data
  and rax, r8			; reset bits
  or rax, r9  			; insert our bits
  mov [rsi+8], rax		; put back
  RET
