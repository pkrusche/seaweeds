...
#include "windowwindow/WindowOutput.h"
	...
	public:
		...
		bool parsecommandline (int argc, char** argv) {
			...
			desc.add_options()
				...
				(	"method,m", 
					...
					"choose method to use: [lcs|blcs|seaweeds|scores|scoresoverlap|needlemanwunsch]" )
				(	"overlap,v", 
				...
				(	"substitutionmatrix,X", 
					po::value<string> (&substitutionmatrix)->default_value("ABGCTxNz"), 
					"specify substitution matrix file name. default scores are 1/0/-0.5 for match/mismatch/gap")
			;
		...
		void setoutput(std::string const & outputidentifier, int offset = 0, int p_offset = 0) {
			...
			if(full_output) {
				...
				// LCS
				if(method != NEEDLEMAN_WUNSCH) {
					tp = new WindowOutputUnbuffered<string>(*output, winlen, winlen, cutoffthreshold, 
						get_profile_size_a(), 
						get_profile_size_b(), 
						firststepwidth, secondstepwidth);
				} else {
					tp = new WindowOutputUnbuffered<string, lcs::NoTranslation<string> >(*output, winlen, winlen, cutoffthreshold, 
						get_profile_size_a(), 
						get_profile_size_b(), 
						firststepwidth, secondstepwidth);
				}
				...
				if(method != NEEDLEMAN_WUNSCH) {
					tp = new WindowOutputWithBuffer<string>(*output, 
						buffersize,
					...
					tp = new WindowOutputWithBuffer<string, lcs::NoTranslation<string> >(*output, 
						buffersize,
						winlen, winlen, cutoffthreshold, 
						get_profile_size_a(), 
						get_profile_size_b(), 
		...
		private:
			...
			WindowPairOutput<string> * tp; ///< the output printing helper.
