...
public:
	...
	static const UINT64 lsbs = ( msb-1 );
   	...
   	BoassonMPRAMMatcher(size_t _window, string const & _pattern) 
		: window(_window) {
		using namespace std;
	...
	int count(string const & text, 
		...
		for(size_t pos = 0; pos < n; ++pos) {
			...
			lcslen = min((int)patsize, lcslen+1);
			...
			if( pos >= window-1 ) {
				if(rpt != NULL) {
					rpt->report_score(windowlocal::window((int)(pos+1-window+text_p0), 
											pat_p0, (double)lcslen));
				}
