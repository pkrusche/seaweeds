...
Parameter::Parameter() : type(EMPTY), 
	updated(false), len(0), last_update(bsp_time()) {}

Parameter::Parameter(int value) : type(INT), 
	data(new char[sizeof(uint64_t)]), 
	len(sizeof(uint64_t)), last_update(bsp_time()) {
	*((int64_t*) data.get()) = value;
	updated = true;
}

Parameter::Parameter(double value) : type(FLOAT), data(new char[sizeof(double)]), len(sizeof(double)), last_update(bsp_time()) {
...
void ParameterFile::get(const char * name, double & val, double def) {
    _iterator it = m_Params.find(prefix + string(name));
    val= def;
	if(it != m_Params.end()) {
...
void ParameterFile::set(const char * name, std::string const & val) {
	if(m_Params.find(name) == m_Params.end() || m_Params[prefix + string(name)].as_string() != val) {
		m_Params[prefix + string(name)] = Parameter(val.c_str());
		notify(name, &(m_Params[prefix + string(name)]), "set");
	}
}

void ParameterFile::set(const char * name, int val) {
	std::string pname = prefix + string(name);
...
void ParameterFile::set(const char * name, double val) {
	m_Params[prefix + string(name)] = Parameter(val);
	notify(name, &(m_Params[prefix + string(name)]), "set");
}

void ParameterFile::set(const char * name, Parameter val) {
	m_Params[prefix + string(name)] = val;
	notify(name, &(m_Params[prefix + string(name)]), "set");
}
...
void ParameterFile::setprefix(const char *_prefix) {
	prefix = std::string(_prefix == NULL ? "" : _prefix);
}
