...
namespace utilities {
	...
	*/
	...
	public:
		...
		static const UINT64 lsbs = (static_cast<UINT64>(2))*msb - 1;
		...
		*/
		 ...
		 * Copy constructor
		 */
		IntegerVector(my_type const & v) : vword_len(0), is_slice(false) {
			value_mask = bitmasks_64[value_bits];
			_carry = false;
		...
		*/
		void zero() {
			if(vword_len > 0) {
				A_memset(content.data, 0, sizeof(UINT64)*content.size);
			}
		}
		...
		void one() {
			if(vword_len > 0) {
				A_memset(content.data, 0xff, sizeof(UINT64)*content.size);
				fixending();
			}
		}
		...
		*/
		...
		my_type & operator+= (my_type const& v) {
			...
			if( (vword_len*value_bits & 0x3f) != 0) {
				...
				UINT64 mask = bits_64[last_bit_ofs()];
		...
		*/
		...
		/**
		* compare-exchange: exchanges elements 
		* (*this)[i] and t[i] iff. 
		*     (*this)[i] > t[i] || m[i]
		* (seaweed core operation)
		*/
		void cmpxchg_masked(my_type & t,  my_type & m) {
			using namespace std;
			ASSERT(vword_len == t.vword_len);
			ASSERT(vword_len == m.vword_len);
			for (size_t j= 0; j < vword_len; ++j) {
				UINT64 tmp1 = (*this)[j];
				UINT64 tmp2 = t[j];
				UINT64 tmp3 = m[j];
		...
		*/
		 ...
		 */
		void saturated_inc() {
			static const UINT64 vmax = 2*(static_cast<UINT64>(msb))-1;
			for (size_t i = 0; i < vword_len; ++i)	{
				UINT64 v = (*this)[i];
				if (v < vmax)
				{
					v++;
	...
	private:
		inline BYTE last_bit_ofs() {
			return (BYTE) (((vword_len)*((size_t)value_bits)) & 0x3f);
		}

		inline size_t last_relevant() {
			return last_bit_ofs() == 0 ? vwords_toUINT64s_lb(vword_len-1) 
				: vwords_toUINT64s_lb(vword_len);
		}

		/** number of virtual words to machine words (ceiling) */
		inline size_t vwords_toUINT64s(size_t lwords) const {
			return (size_t) ( ( ( ( ((UINT64) lwords)* ((UINT64)value_bits) )) >> 6 ) + 2 );
		}

		/** number of virtual words to machine words (floor) */
		inline size_t vwords_toUINT64s_lb(size_t lwords) const {
			return ( (lwords*value_bits) >> 6 );
		}

		/** bit offset of a given word */
		inline BYTE bitofs(size_t ofs) const {
		...
		BYTE _carry; ///< carry after addition
	...
	template<BYTE value_bits>
	std::istream & operator>> (std::istream & i, IntegerVector<value_bits> & mvv) {
		using namespace std;
		...
		if(value_bits > 3) {
			...
			while (cont) {
				...
				if (!getline(i, str, "|\n\t ")) {
					...
					UINT64 val = strtol(str.c_str(), &end, 16);
					vals.push_back(val);
				}
			}
		} else {
			...
			while ( cont && i.good() ) {
				i.get(c[0]);
				if (!i.good() || strchr("0123456789abcdefABCDEF", c[0]) == NULL) {
					cont = false;
				} else {
					UINT64 val = strtol(c, &end, 16);
					vals.push_back(val);
				}
			}
		}
		mvv.resize(vals.size());
...
#include "IntegerVectorBinarySerialization.h"

// Specialised implementations using vectorclass
// Currently, there is one for cmpxchg_mask
#ifdef _USE_VECTORCLASS
#include "IntegerVector_vectorclass.inl"
#endif
