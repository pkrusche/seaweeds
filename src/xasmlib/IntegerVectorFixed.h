 ...
 */
	...
	template<> inline IntegerVector<8> & IntegerVector<8>::operator+= (UINT64 j) {
	...
	template <> inline void IntegerVector<8>::bittest(int c) {
		functors::elementwise_operation<BYTE, functors::_bt> ((BYTE *)content.data, (BYTE)c, vword_len);
	}

	template<> inline IntegerVector<16> & IntegerVector<16>::operator+= (UINT64 j) {
	...
	template <> inline void IntegerVector<16>::bittest(int c) {
		functors::elementwise_operation<WORD, functors::_bt> ((WORD *)content.data, (WORD)c, vword_len);
	}


	template<> inline IntegerVector<32> & IntegerVector<32>::operator+= (UINT64 j) {
	...
	template <> inline void IntegerVector<32>::bittest(int c) {
		functors::elementwise_operation<DWORD, functors::_bt> ((DWORD *)content.data, (DWORD)c, vword_len);
	}

	template <> inline void IntegerVector<32>::saturated_inc() {
		functors::elementwise_operation_1<DWORD, functors::_incs> ((DWORD *)content.data, vword_len);
	}

	template <> inline void IntegerVector<8>::put(size_t pos, int value) {
		((BYTE*)content.data)[pos] = (BYTE)value;
	}

	template <> inline int IntegerVector<8>::get(size_t pos) const {
		return ((const BYTE*)content.data)[pos];
	}

	template <> inline  void IntegerVector<8>::fixending() {
		A_memset(((BYTE*)content.data) + vword_len, 0, 8*content.size-vword_len);
	}

	template <>  inline void IntegerVector<16>::fixending() {
		memset(((WORD*)content.data) + vword_len, 0, 8*content.size-(2*vword_len));
	}

	template <> inline void IntegerVector<32>::fixending() {
		memset(((DWORD*)content.data) + vword_len, 0, 8*content.size-(4*vword_len));
	}

#ifndef _USE_VECTORCLASS
	/* specialised seaweed function */
	template <> inline void IntegerVector<8>::cmpxchg_masked(IntegerVector<8> & t, IntegerVector<8> & m) {
		cmpxchg_with_mask_8(content.data, t.content.data, m.content.data, vword_len);		
	}
#endif

	/** SIMD accelerated specialisations. */
#ifdef _HAVE_SIMD
	template <> inline void IntegerVector<8>::saturated_inc() {
		vecsatinc_8_mmx(content.data, content.size);
	}

	template<> inline void IntegerVector<8>::generate_match_mask
...
//			fixending();
	...
	template<> inline void IntegerVector<8>::cmpxchg(IntegerVector<8> & t) {
		using namespace std;
		ASSERT(vword_len == t.vword_len);
		cmpxchg_8_mmx(content.data, t.content.data, content.size);
	}

	template <> inline void IntegerVector<16>::saturated_inc() {
		vecsatinc_16_mmx(content.data, content.size);
	}

	template<> inline void IntegerVector<16>::generate_match_mask
...
//			fixending();
	...
	* (*this)[i] > t[i]
	*/
	template<> inline void IntegerVector<16>::cmpxchg(IntegerVector<16> & t) {
		using namespace std;
		ASSERT(vword_len == t.vword_len);
